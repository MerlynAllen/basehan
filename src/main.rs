pub mod BaseHan {
    ...
    pub const MULTIBYTE_SIGN: u32 = 0x4dff;
    ...
    pub fn decode(basehan: String) -> Vec<u8> {
        ...
        for i in 0..basehan.len() - 1 {
            let mut index = basehan[i] as u32 - BASE_OFFSET;
        ...
        assert!(bit_pointer + tail_bits == 8);
        let byte = buff & 0xFF;
        result.push(byte as u8);
        result
    }
...
fn main() {
    ...
    if args.decode {
        ...
        let result = BaseHan::encode(buffer.as_bytes().to_vec());
